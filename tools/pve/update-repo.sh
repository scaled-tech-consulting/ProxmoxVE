...
function update_container() {
  ...
  if [[ "$os" == "ubuntu" || "$os" == "debian" ]]; then
    ...
    if pct exec "$container" -- [ -e /usr/bin/update ]; then
      if pct exec "$container" -- grep -q "scaled-tech-consulting/ProxmoxVE" /usr/bin/update; then
        ...
        pct exec "$container" -- bash -c "sed -i 's/tteck\\/Proxmox/scaled-tech-consulting\\/ProxmoxVE/g' /usr/bin/update"
...
header_info
echo -e "${GN}The process is complete. The repositories have been switched to scaled-tech-consulting/ProxmoxVE.${CL}\n"
